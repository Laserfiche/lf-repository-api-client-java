...
public interface ServerSessionClient {
   ...
   * provided access token. This method should be used when the client wants to clean up the current
   * session.<br>
  ...
  /**
   * - Deprecated.<br>
   * - Refreshes the session associated with the access token. This is only necessary if you want to
   * keep the same session alive, otherwise a new session will be automatically created when the
   * session expires.<br>
   * - When a client application wants to keep a session alive that has been idle for an hour, this
   * route can be used to refresh the expiration timer associated with the access token.<br>
   * - Only available in Laserfiche Cloud.
   *
