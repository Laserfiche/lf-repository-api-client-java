...
public interface EntriesClient {
     ...
     * - Provide the new field values to assign to the entry, and remove/reset all previously
     * assigned field values. <br>
     * - This is an overwrite action. The request body must include all desired field values,
     * including any existing field values that should remain assigned to the entry. Field values
     * that are not included in the request will be deleted from the entry. If the field value that
     * is not included is part of a template, it will still be assigned (as required by the
     * template), but its value will be reset.
     *
     ...
     * during the creation. As long as the entryCreate operation succeeds, the entry will be
     * created, even if all other operations fail.
     *
     ...
     * <br>
     * - Only template values will be modified. Any existing independent fields on the entry will
     * not be modified, nor will they be added if included in the request. The only modification to
     * fields will only occur on templated fields. If the previously assigned template includes
     * common template fields as the newly assigned template, the common field values will not be
     * modified.
     *
     ...
     * - Optional query parameter: fallbackToClosestAncestor. Use the fallbackToClosestAncestor
     * query parameter to return the closest existing ancestor if the initial entry path is not
     * found.
     *
     ...
     * - Move and/or rename an entry by passing in the new parent folder ID or name in the JSON
     * body.<br>
     * - Optional parameter: autoRename (default false). If an entry already exists with the given
     ...
     * - Provide the parent folder ID, and based on the request body, copy or create a
     * folder/shortcut as a child entry of the designated folder.<br>
     * - Optional parameter: autoRename (default false). If an entry already exists with the given
     * name, the entry will be automatically renamed.
     *
