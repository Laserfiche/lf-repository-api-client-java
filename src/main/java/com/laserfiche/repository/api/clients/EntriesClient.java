...
public interface EntriesClient {
   ...
   *
   * @param parameters An object of type {@link ParametersForGetFieldValues} which encapsulates the
   *     parameters of {@link #getFieldValues getFieldValues} method.
  ...
  /**
   * - Update the field values assigned to an entry. - Provide the new field values to assign to the
   * entry, and remove/reset all previously assigned field values. - This is an overwrite action.
   * The request body must include all desired field values, including any existing field values
   * that should remain assigned to the entry. Field values that are not included in the request
   * will be deleted from the entry. If the field value that is not included is part of a template,
   * it will still be assigned (as required by the template), but its value will be reset.
   *
  ...
  /**
   * - Creates a new document in the specified folder with file (no more than 100 MB). - Optionally
   * sets metadata and electronic document component. - Optional parameter: autoRename (default
   * false). If an entry already exists with the given name, the entry will be automatically
   * renamed. With this route, partial success is possible. The response returns multiple operation
   * (entryCreate operation, setEdoc operation, setLinks operation, etc..) objects, which contain
   * information about any errors that may have occurred during the creation. As long as the
   * entryCreate operation succeeds, the entry will be created, even if all other operations fail.
   *
   * @param parameters An object of type {@link ParametersForImportDocument} which encapsulates the
   *     parameters of {@link #importDocument importDocument} method.
  ...
  /**
   * - Assign links to an entry. - Provide an entry ID and a list of links to assign to that entry.
   * - This is an overwrite action. The request must include all links to assign to the entry,
   * including existing links that should remain assigned to the entry.
   *
   ...
   * independent fields on the entry will not be modified, nor will they be added if included in the
   * request. The only modification to fields will only occur on templated fields. If the previously
   * assigned template includes common template fields as the newly assigned template, the common
   * field values will not be modified.
   *
   * @param parameters An object of type {@link ParametersForWriteTemplateValueToEntry} which
   *     encapsulates the parameters of {@link #writeTemplateValueToEntry writeTemplateValueToEntry}
   *     method.
   ...
   *     encapsulates the parameters of {@link #getDynamicFieldValues getDynamicFieldValues} method.
   ...
   *
   * @param parameters An object of type {@link ParametersForGetEntryByPath} which encapsulates the
   *     parameters of {@link #getEntryByPath getEntryByPath} method.
   ...
   * operationToken. - Provide the parent folder ID, and copy an entry as a child of the designated
   * folder. - Optional parameter: autoRename (default false). If an entry already exists with the
   * given name, the entry will be automatically renamed. - The status of the operation can be
   * checked via the Tasks/{operationToken} route. - Token substitution in the name of the copied
   * entry is not supported.
   *
   * @param parameters An object of type {@link ParametersForCopyEntryAsync} which encapsulates the
   *     parameters of {@link #copyEntryAsync copyEntryAsync} method.
   * @return {@link AcceptedOperation} The return value
   */
  AcceptedOperation copyEntryAsync(ParametersForCopyEntryAsync parameters);

  /**
   * - Returns a single entry object. - Provide an entry ID, and get the entry associated with that
   * ID. Useful when detailed information about the entry is required, such as metadata, path
   ...
   * queue a delete task to remove it from the repository (includes nested objects if the entry is a
   * Folder type). The entry will not be deleted immediately. - Optionally include an audit reason
   * ID and comment in the JSON body. This route returns an operationToken, and will run as an
   * asynchronous operation. Check the progress via the Tasks/{operationToken} route.
   *
   * @param parameters An object of type {@link ParametersForDeleteEntryInfo} which encapsulates the
   *     parameters of {@link #deleteEntryInfo deleteEntryInfo} method.
   * @return {@link AcceptedOperation} The return value
   */
   ...
   * Provide an entry ID and audit reason/comment in the request body, and get the edoc resource as
   * part of the response content. - Optional header: Range. Use the Range header (single range with
   * byte unit) to retrieve partial content of the edoc, rather than the entire edoc. This route is
   * identical to the GET edoc route, but allows clients to include an audit reason when downloading
   * the edoc.
   *
  ...
  /**
   * - Returns an entry's edoc resource in a stream format. - Provide an entry ID, and get the edoc
   * resource as part of the response content. - Optional header: Range. Use the Range header
   * (single range with byte unit) to retrieve partial content of the edoc, rather than the entire
   * edoc.
   *
   * @param parameters An object of type {@link ParametersForExportDocument} which encapsulates the
   *     parameters of {@link #exportDocument exportDocument} method.
   */
   ...
   *
   * @param parameters An object of type {@link ParametersForDeleteDocument} which encapsulates the
   *     parameters of {@link #deleteDocument deleteDocument} method.
   ...
   * response headers. - This route does not provide a way to download the actual edoc. Instead, it
   * just gives metadata information about the edoc associated with the entry. - If an error occurs,
   * the error message can be found in the X-APIServer-Error HTTP response header.
   *
  ...
  /**
   * - Returns the children entries of a folder in the repository. - Provide an entry ID (must be a
   * folder), and get a paged listing of entries in that folder. Used as a way of navigating through
   * the repository. - Default page size: 150. Allowed OData query options: Select | Count | OrderBy
   * | Skip | Top | SkipToken | Prefer. OData $OrderBy syntax should follow: &quot;PropertyName
   * direction,PropertyName2 direction&quot;. Sort order can be either value &quot;asc&quot; or
   * &quot;desc&quot;. Optional query parameters: groupByOrderType (bool). This query parameter
   * decides if results are returned in groups based on their entry type. Entries returned in the
   * listing are not automatically converted to their subtype (Folder, Shortcut, Document), so
   * clients who want model-specific information should request it via the GET entry by ID route. -
   * Optionally returns field values for the entries in the folder. Each field name needs to be
   ...
   *
   * @param parameters An object of type {@link ParametersForGetEntryListing} which encapsulates the
   *     parameters of {@link #getEntryListing getEntryListing} method.
   ...
   * based on the request body, copy or create a folder/shortcut as a child entry of the designated
   * folder. - Optional parameter: autoRename (default false). If an entry already exists with the
   * given name, the entry will be automatically renamed.
   *
  ...
  /**
   * - Returns the tags assigned to an entry. - Provide an entry ID, and get a paged listing of tags
   * assigned to that entry. - Default page size: 100. Allowed OData query options: Select | Count |
   * OrderBy | Skip | Top | SkipToken | Prefer.
   *
