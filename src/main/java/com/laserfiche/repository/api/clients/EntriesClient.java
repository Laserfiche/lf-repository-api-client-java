...
public interface EntriesClient {
    ...
    /**
     * - Returns the fields assigned to an entry.<br>- Provide an entry ID, and get a paged listing of all fields assigned to that entry.<br>- Default page size: 100. Allowed OData query options: Select | Count | OrderBy | Skip | Top | SkipToken | Prefer.
     *
    ...
    /**
     * - Returns the links assigned to an entry.<br>- Provide an entry ID, and get a paged listing of links assigned to that entry.<br>- Default page size: 100. Allowed OData query options: Select | Count | OrderBy | Skip | Top | SkipToken | Prefer.
     *
    ...
    /**
     * - Assign links to an entry.<br>- Provide an entry ID and a list of links to assign to that entry.<br>- This is an overwrite action. The request must include all links to assign to the entry, including existing links that should remain assigned to the entry.
     *
    ...
    /**
     * - Remove the currently assigned template from the specified entry.<br>- Provide an entry ID to clear template value on.<br>- If the entry does not have a template assigned, no change will be made.
     *
    ...
    /**
     * - Returns a single entry object using the entry path.<br>- Optional query parameter: fallbackToClosestAncestor. Use the fallbackToClosestAncestor query parameter to return the closest existing ancestor if the initial entry path is not found.
     *
    ...
    /**
     * - Copy a new child entry in the designated folder async, and potentially return an operationToken.<br>- Provide the parent folder ID, and copy an entry as a child of the designated folder.<br>- Optional parameter: autoRename (default false). If an entry already exists with the given name, the entry will be automatically renamed. <br>- The status of the operation can be checked via the Tasks/{operationToken} route.<br>- Token substitution in the name of the copied entry is not supported.
     *
    ...
    /**
     * - Moves and/or renames an entry.<br>- Move and/or rename an entry by passing in the new parent folder ID or name in the JSON body.<br>- Optional parameter: autoRename (default false). If an entry already exists with the given name, the entry will be automatically renamed.
     *
    ...
    /**
     * - Begins a task to delete an entry, and returns an operationToken.<br>- Provide an entry ID, and queue a delete task to remove it from the repository (includes nested objects if the entry is a Folder type). The entry will not be deleted immediately.<br>- Optionally include an audit reason ID and comment in the JSON body. This route returns an operationToken, and will run as an asynchronous operation. Check the progress via the Tasks/{operationToken} route.
     *
    ...
    /**
     * - Returns an entry's edoc resource in a stream format while including an audit reason.<br>- Provide an entry ID and audit reason/comment in the request body, and get the edoc resource as part of the response content.<br>- Optional header: Range. Use the Range header (single range with byte unit) to retrieve partial content of the edoc, rather than the entire edoc. This route is identical to the GET edoc route, but allows clients to include an audit reason when downloading the edoc.
     *
    ...
    /**
     * - Returns an entry's edoc resource in a stream format.<br>- Provide an entry ID, and get the edoc resource as part of the response content.<br>- Optional header: Range. Use the Range header (single range with byte unit) to retrieve partial content of the edoc, rather than the entire edoc.
     *
    ...
    /**
     * - Returns information about the edoc content of an entry, without downloading the edoc in its entirety.<br>- Provide an entry ID, and get back the Content-Type and Content-Length in the response headers.<br>- This route does not provide a way to download the actual edoc. Instead, it just gives metadata information about the edoc associated with the entry.<br>- If an error occurs, the error message can be found in the X-APIServer-Error HTTP response header.
     *
    ...
    /**
     * - Delete the pages associated with the provided entry ID. If no pageRange is specified, all pages will be deleted.<br>- Optional parameter: pageRange (default empty). The value should be a comma-seperated string which contains non-overlapping single values, or page ranges. Ex: &quot;1,2,3&quot;, &quot;1-3,5&quot;, &quot;2-7,10-12.&quot;
     *
    ...
    /**
     * - Create/copy a new child entry in the designated folder.<br>- Provide the parent folder ID, and based on the request body, copy or create a folder/shortcut as a child entry of the designated folder.<br>- Optional parameter: autoRename (default false). If an entry already exists with the given name, the entry will be automatically renamed.
     *
    ...
    /**
     * - Returns the tags assigned to an entry.<br>- Provide an entry ID, and get a paged listing of tags assigned to that entry.<br>- Default page size: 100. Allowed OData query options: Select | Count | OrderBy | Skip | Top | SkipToken | Prefer.
     *
    ...
    /**
     * - Assign tags to an entry.<br>- Provide an entry ID and a list of tags to assign to that entry.<br>- This is an overwrite action. The request must include all tags to assign to the entry, including existing tags that should remain assigned to the entry.
     *
