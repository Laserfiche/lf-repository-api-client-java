...
public interface EntriesClient {
   ...
   * - Provide the new field values to assign to the entry, and remove/reset all previously assigned
   * field values. <br>
   * - This is an overwrite action. The request body must include all desired field values,
   * including any existing field values that should remain assigned to the entry. Field values that
   * are not included in the request will be deleted from the entry. If the field value that is not
   * included is part of a template, it will still be assigned (as required by the template), but
   * its value will be reset.
   *
   ...
   * - Optional parameter: autoRename (default false). If an entry already exists with the given
   * name, the entry will be automatically renamed. With this route, partial success is possible.
   * The response returns multiple operation (entryCreate operation, setEdoc operation, setLinks
   * operation, etc..) objects, which contain information about any errors that may have occurred
   * during the creation. As long as the entryCreate operation succeeds, the entry will be created,
   * even if all other operations fail.
   *
   ...
   * <br>
   * - Only template values will be modified. Any existing independent fields on the entry will not
   * be modified, nor will they be added if included in the request. The only modification to fields
   * will only occur on templated fields. If the previously assigned template includes common
   * template fields as the newly assigned template, the common field values will not be modified.
   *
   ...
   * - Optional query parameter: fallbackToClosestAncestor. Use the fallbackToClosestAncestor query
   * parameter to return the closest existing ancestor if the initial entry path is not found.
   *
   ...
   * - Optional parameter: autoRename (default false). If an entry already exists with the given
   * name, the entry will be automatically renamed. <br>
   ...
   * - Provide an entry ID, and get the entry associated with that ID. Useful when detailed
   * information about the entry is required, such as metadata, path information, etc.<br>
   * - Allowed OData query options: Select. If the entry is a subtype (Folder, Document, or
   * Shortcut), the entry will automatically be converted to include those model-specific
   * properties.
   *
   ...
   * <br>
   * - Optional parameter: autoRename (default false). If an entry already exists with the given
   * name, the entry will be automatically renamed.
   *
   ...
   * - Provide an entry ID, and queue a delete task to remove it from the repository (includes
   * nested objects if the entry is a Folder type). The entry will not be deleted immediately.<br>
   * - Optionally include an audit reason ID and comment in the JSON body. This route returns an
   * operationToken, and will run as an asynchronous operation. Check the progress via the
   * Tasks/{operationToken} route.
   *
   ...
   * as part of the response content.<br>
   * - Optional header: Range. Use the Range header (single range with byte unit) to retrieve
   * partial content of the edoc, rather than the entire edoc. This route is identical to the GET
   * edoc route, but allows clients to include an audit reason when downloading the edoc.
   *
   ...
   * - Optional header: Range. Use the Range header (single range with byte unit) to retrieve
   * partial content of the edoc, rather than the entire edoc.
   *
   ...
   * entirety.<br>
   * - Provide an entry ID, and get back the Content-Type and Content-Length in the response
   * headers.<br>
   * - This route does not provide a way to download the actual edoc. Instead, it just gives
   * metadata information about the edoc associated with the entry.<br>
   * - If an error occurs, the error message can be found in the X-APIServer-Error HTTP response
   * header.
   *
   ...
   * pages will be deleted.<br>
   * - Optional parameter: pageRange (default empty). The value should be a comma-seperated string
   * which contains non-overlapping single values, or page ranges. Ex: &quot;1,2,3&quot;,
   * &quot;1-3,5&quot;, &quot;2-7,10-12.&quot;
   *
   ...
   * Used as a way of navigating through the repository.<br>
   ...
   * specified in the request. Maximum limit of 10 field names.<br>
   ...
   * - Provide the parent folder ID, and based on the request body, copy or create a folder/shortcut
   * as a child entry of the designated folder.<br>
   * - Optional parameter: autoRename (default false). If an entry already exists with the given
   * name, the entry will be automatically renamed.
   *
