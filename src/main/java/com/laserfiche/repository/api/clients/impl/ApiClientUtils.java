...
public class ApiClientUtils {
  private ApiClientUtils() {
    throw new IllegalStateException(
        "Utility class with all static methods are not meant to be instantiated.");
  }
  ...
  public static String getCreateEntryResultSummary(CreateEntryResult createEntryResult) {
    ...
    if (createEntryResult != null && createEntryResult.getOperations() != null) {
      ...
      if (operations.getEntryCreate() != null) {
        ...
        messages.add(
            getErrorMessagesFromAPIServerExceptions(operations.getEntryCreate().getExceptions()));
      }
      if (operations.getSetEdoc() != null)
        messages.add(
            getErrorMessagesFromAPIServerExceptions(operations.getSetEdoc().getExceptions()));
      if (operations.getSetTemplate() != null)
        messages.add(
            getErrorMessagesFromAPIServerExceptions(operations.getSetTemplate().getExceptions()));
      if (operations.getSetFields() != null)
        messages.add(
            getErrorMessagesFromAPIServerExceptions(operations.getSetFields().getExceptions()));
      if (operations.getSetTags() != null)
        messages.add(
            getErrorMessagesFromAPIServerExceptions(operations.getSetTags().getExceptions()));
      if (operations.getSetLinks() != null)
        messages.add(
            getErrorMessagesFromAPIServerExceptions(operations.getSetLinks().getExceptions()));
    }
    return messages.stream().filter(Objects::nonNull).collect(Collectors.joining(" ")).trim();
  }

  private static String getErrorMessagesFromAPIServerExceptions(
      Collection<APIServerException> errors) {
  ...
  protected static ApiException createApiException(
      HttpResponse<?> httpResponse, ProblemDetails problemDetails) {
  ...
  public static <TResponse> TResponse sendRequestWithRetry(
    ...
    while (retryCount <= maxRetries && shouldRetry) {
      try {
        ...
        final HttpRequestWithBody httpRequestWithBody =
            httpClient.request(requestMethod, requestUrl);
