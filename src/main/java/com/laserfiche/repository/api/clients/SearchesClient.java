...
public interface SearchesClient {
    ...
    /**
     * - Returns search status. - Provide a token (returned in the create search asynchronous
     * route), and get the search status, progress, and any errors that may have occurred. When the
     * search is completed, the Location header can be inspected as a link to the search results. -
     ...
     * considered a match (integer value for NumberOfLetters, or int value representing a
     * percentage). The status for search operations must be checked via the Search specific status
     * checking route.
     *
     ...
     * false). If the search listing should be refreshed to show updated values. - Default page
     * size: 150. Allowed OData query options: Select | Count | OrderBy | Skip | Top | SkipToken |
     * Prefer. OData $OrderBy syntax should follow: &quot;PropertyName direction,PropertyName2
     ...
     * returns field values for the entries in the search result listing. Each field name needs to
     * be specified in the request. Maximum limit of 10 field names. - If field values are
