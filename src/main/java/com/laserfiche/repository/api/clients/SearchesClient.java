...
public interface SearchesClient {
    ...
    /**
     * - Returns search status.<br>- Provide a token (returned in the create search asynchronous route), and get the search status, progress, and any errors that may have occurred. When the search is completed, the Location header can be inspected as a link to the search results.<br>- OperationStatus can be one of the following : NotStarted, InProgress, Completed, Failed, or Canceled.
     *
    ...
    /**
     * - Cancels a currently running search.<br>- Closes a completed search.
     *
    ...
    /**
     * - Returns the context hits associated with a search result entry.<br>- Given a searchToken, and rowNumber associated with a search entry in the listing, return the context hits for that entry.<br>- Default page size: 100. Allowed OData query options: Select | Count | OrderBy | Skip | Top | SkipToken | Prefer.
     *
    ...
    /**
     * - Returns a search result listing if the search is completed.<br>- Optional query parameter: groupByOrderType (default false). This query parameter decides whether or not results are returned in groups based on their entry type.<br>- Optional query parameter: refresh (default false). If the search listing should be refreshed to show updated values.<br>- Default page size: 150. Allowed OData query options: Select | Count | OrderBy | Skip | Top | SkipToken | Prefer. OData $OrderBy syntax should follow: &quot;PropertyName direction,PropertyName2 direction&quot;. sort order can be either &quot;asc&quot; or &quot;desc&quot;. Search results expire after 5 minutes, but can be refreshed by retrieving the results again.<br>- Optionally returns field values for the entries in the search result listing. Each field name needs to be specified in the request. Maximum limit of 10 field names.<br>- If field values are requested, only the first value is returned if it is a multi value field.<br>- Null or Empty field values should not be used to determine if a field is assigned to the entry.
     *
