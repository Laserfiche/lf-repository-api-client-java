...
public interface AttributesClient {
     ...
     *
     * @param parameters An object of type {@link ParametersForGetTrusteeAttributeValueByKey} which encapsulates the parameters of {@link #getTrusteeAttributeValueByKey getTrusteeAttributeValueByKey} method.
    ...
    /**
     * - Returns the attribute key value pairs associated with the authenticated user. Alternatively, return only the attribute key value pairs that are associated with the &quot;Everyone&quot; group.<br>- Attribute keys can be used with subsequent calls to get specific attribute values.<br>- Default page size: 100. Allowed OData query options: Select, Count, OrderBy, Skip, Top, SkipToken, Prefer. Optional query parameters: everyone (bool, default false). When true, this route does not return the attributes that are tied to the currently authenticated user, but rather the attributes assigned to the &quot;Everyone&quot; group. Note when this is true, the response does not include both the &quot;Everyone&quot; groups attribute and the currently authenticated user, but only the &quot;Everyone&quot; groups.
     *
    ...
    /**
     * Provides the functionality to iteratively (i.e. through paging) call {@link #getTrusteeAttributeKeyValuePairs getTrusteeAttributeKeyValuePairs}, and apply a function on the response of each iteration.
     *
