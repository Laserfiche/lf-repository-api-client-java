...
public class SetEntriesApiTest extends BaseTest {
    ...
    void setTags_ReturnTags() {
        ODataValueContextOfIListOfWTagInfo tagDefinitionsResponse =
                repositoryApiClient
                        .getTagDefinitionsClient()
                        .getTagDefinitions(
                                new ParametersForGetTagDefinitions().setRepoId(repositoryId));
        ...
        Entry entry =
                createEntry(
                        client,
                        "RepositoryApiClientIntegrationTest Java SetTags",
                        testClassParentFolder.getId(),
                        true);
        Integer num = entry.getId();

        ODataValueOfIListOfWTagInfo assignTagsResponse =
                repositoryApiClient
                        .getEntriesClient()
                        .assignTags(
                                new ParametersForAssignTags()
    ...
    void setTemplates_ReturnTemplates() throws ExecutionException, InterruptedException {
        ...
        for (WTemplateInfo templateDefinition : templateDefinitions) {
            ODataValueContextOfIListOfTemplateFieldInfo templateDefinitionsFieldsResponse =
                    repositoryApiClient
                            .getTemplateDefinitionClient()
            ...
            if (templateDefinitionsFieldsResponse.getValue() != null
                    && noRequiredFieldDefinitionsInTemplate(
        ...
        Entry entry =
                createEntry(
                        client,
                        "RepositoryApiClientIntegrationTest Java DeleteTemplate",
                        testClassParentFolder.getId(),
                        true);

        Entry setTemplateResponse =
                repositoryApiClient
                        .getEntriesClient()
    ...
    void setFields_ReturnFields() {
        ...
        ODataValueContextOfIListOfWFieldInfo fieldDefinitionsResponse =
                repositoryApiClient
        ...
        for (WFieldInfo fieldDefinition : fieldDefinitions) {
            if (fieldDefinition.getFieldType().equals(WFieldType.STRING)
        ...
        Entry entry =
                createEntry(
                        client,
                        "RepositoryApiClientIntegrationTest Java SetFields",
                        testClassParentFolder.getId(),
                        true);
        Integer entryId = entry.getId();

        ODataValueOfIListOfFieldValue assignFieldValuesResponse =
                repositoryApiClient
                        .getEntriesClient()
                        .assignFieldValues(
                                new ParametersForAssignFieldValues()
                                        .setRepoId(repositoryId)
                                        .setEntryId(entryId)
                                        .setRequestBody(requestBody));
        List<FieldValue> fields = assignFieldValuesResponse.getValue();
    ...
    void removeTemplateFromEntry_ReturnEntry() throws ExecutionException, InterruptedException {
        ...
        Entry writeTemplateValueToEntryResponse =
