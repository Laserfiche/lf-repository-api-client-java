...
public class BaseTest {
  ...
  public static void setUp() {
    Dotenv dotenv = Dotenv.configure().filename(".env").systemProperties().ignoreIfMissing().load();
    try {
  ...
  public static void WaitUntilSearchEnds(
      AcceptedOperation search, Duration interval, Duration timeout) throws InterruptedException {
    int maxIteration = (int) (timeout.toMillis() / interval.toMillis());
    int count = 0;
    while (count < maxIteration) {
      OperationProgress progress =
